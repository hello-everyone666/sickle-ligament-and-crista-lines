...
def parse_args():
    ...
    parser.add_argument('--epoch', default=20000, type=int, help='epoch to run')
    parser.add_argument('--learning_rate', default=0.001, type=float, help='initial learning rate')
    parser.add_argument('--gpu', type=str, default='4', help='specify GPU devices')
...
def main(args):
    ...
    start_epoch=0
    if args.optimizer == 'Adam':#Adam优化器
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            shape_ious = {cat: [] for cat in seg_classes.keys()}#dict:1
            seg_label_to_cat = {}  # {0:background,1:ridge,2:ligament}

            for cat in seg_classes.keys():#1个物体类别
                for label in seg_classes[cat]:#3个部件类别
                    seg_label_to_cat[label] = cat
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), smoothing=0.9):
                ...
                for l in range(num_part):
                    ...
                    total_correct_class[l] += (np.sum((cur_pred_val == l) & (target == l)))#list:3

                for i in range(cur_batch_size):
                    segp = cur_pred_val[i, :]
                    segl = target[i, :]
                    cat = seg_label_to_cat[segl[0]]#类别字符串，如’liver'
        ...
        if (test_metrics['inctance_avg_iou'] >= best_inctance_avg_iou):
            ...
            torch.save(state, savepath)#保存最优性能模型
